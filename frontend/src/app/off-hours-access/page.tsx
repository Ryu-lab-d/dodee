...
type Stage = 'loading' | 'reason' | 'reading' | 'signing' | 'success' | 'transitioning';
...
export default function OffHoursAccessPage() {
  ...
  }, [loading, attendanceLoading, user, status, stage, router]);

  const handleReasonNext = () => {
    if (!reason.trim()) {
      setError('กรุณาระบุเหตุผลที่ต้องการเข้าถึงข้อมูล');
      return;
    }
    setError(null);
    api
      .get<{ version: string; text: string }>('/attendance/off-hours-agreement')
      .then((t) => {
        setTerms(t);
        setStage('reading');
      })
  ...
  const handleSign = async (blob: Blob) => {
    ...
    try {
      ...
      await api.post('/attendance/off-hours-access', {
        reason: reason.trim(),
        agreedRead: checkedRead,
        agreedComply: checkedComply,
        signatureUrl: url,
      });
      await refresh();
      setStage('success');
  ...
  if (stage === 'loading' || loading || attendanceLoading) {
    return <LoadingScreen />;
  }

  if (stage === 'transitioning') {
    return <LoadingScreen message="กำลังพาคุณเข้าสู่ระบบ..." />;
  }
    ...
    <div className="flex flex-1 justify-center bg-sky-50 px-4 py-8">
      <div className="w-full max-w-2xl">
        <div className="mb-5 flex flex-col items-center">
          ...
          <p className="mt-2 text-sm text-slate-500">ขอเข้าถึงข้อมูลนอกเวลาทำงาน, {user?.name}</p>
        </div>

        {stage === 'reason' && (
          <div className="card-in rounded-3xl border border-blue-100 bg-white p-6 shadow-sm">
            <h1 className="mb-1 text-lg font-semibold text-slate-900">เหตุผลที่ต้องการเข้าถึงข้อมูล</h1>
            <p className="mb-4 text-sm text-slate-500">กรุณาระบุว่าต้องการเข้าไปทำอะไรในเวลานี้ เหตุผลนี้จะถูกบันทึกไว้ในระบบ</p>
            ...
            <textarea
              ...
              placeholder="เช่น ต้องตรวจสอบข้อมูลผู้เช่าห้อง 101 ให้ลูกค้าด่วน"
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button type="button" onClick={logout} className="rounded-xl px-4 py-3 text-sm font-medium text-slate-400 hover:bg-slate-50">
                ยกเลิก
              </button>
            </div>
          </div>
        ...
        {stage === 'reading' && terms && (
          <div className="card-in rounded-3xl border border-blue-100 bg-white p-6 shadow-sm">
            <h1 className="mb-1 text-lg font-semibold text-slate-900">เงื่อนไขการเข้าถึงข้อมูลนอกเวลา</h1>
            <p className="mb-4 text-sm text-slate-500">กรุณาอ่านเงื่อนไขทั้งหมดก่อนดำเนินการต่อ (ฉบับที่ {terms.version})</p>
            ...
            <div className="mt-5 space-y-3">
              <label className="flex items-start gap-3 text-sm text-slate-700">
                ...
                />
                ฉันได้อ่านข้อมูลครบถ้วน
              </label>
              <label className="flex items-start gap-3 text-sm text-slate-700">
                <input
                  type="checkbox"
                  checked={checkedComply}
                  onChange={(e) => setCheckedComply(e.target.checked)}
                  className="mt-0.5 h-5 w-5 shrink-0 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
                ฉันได้ยอมรับทุกข้อที่ถูกกล่าวถึงและจะปฏิบัติอย่างเคร่งครัด
              </label>
            </div>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                disabled={!checkedRead || !checkedComply}
                className="flex-1 rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40"
              >
                ถัดไป
              </button>
              <button type="button" onClick={() => setStage('reason')} className="rounded-xl px-4 py-3 text-sm font-medium text-slate-400 hover:bg-slate-50">
                ย้อนกลับ
              </button>
            </div>
          </div>
      ...
      {stage === 'success' && (
        <AutoDismissSuccess
          message="บันทึกการเข้าถึงข้อมูลนอกเวลาเรียบร้อยแล้ว"
          onDone={() => {
